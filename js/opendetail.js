...
for (let i = 0; i < clickPicktures.length; i++) {
    ...
    clickPickture.addEventListener('click', (evt) => {
        ...
        evt.preventDefault();
        popupPicture.querySelector('img').src = clickPickture.querySelector('img').src
        popupPicture.querySelector('.likes-count').textContent = clickPickture.querySelector('.picture__likes').textContent
...
let hiddenComments = popupPicture.querySelector('.social__comment-count').classList.add('hidden')
let hiddenUploads = popupPicture.querySelector('.comments-loader').classList.add('hidden')
